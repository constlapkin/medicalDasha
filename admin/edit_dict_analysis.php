...
$dict = R::findOne('dictanalysis', 'WHERE id = :id',
    array(
        ':id' => $id
    ));
...
if (isset($data['submit_edit'])) {
    ...
    if (empty($errors)) {
        $dict = R::load('dictanalysis', $data['id']);
        $dict->type = $data['type'];
        R::store($dict);
        header('Location: admin_dict_analysis.php');
    } else {
...
if ($id and isset($_SESSION['logged_user'])):
    ?>
    <div class="container">
        <p class="back"><a href="admin_dict_analysis.php"><i data-feather="chevron-left"></i>Back</a></p>
        <br>
        <form action="edit_dict_analysis.php" method="post">
